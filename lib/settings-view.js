...
export function observeThemeView () {
  return atom.workspace.onDidStopChangingActivePaneItem(view => {
    ...
    if (isSettingsView(view)) {
      ...
      // disposable.dispose()
      view.addCorePanel(PACKAGE_NAME.replace(/\-(\w)/g, (_, letter) => letter.toUpperCase()), 'rocket', getView)
...
export function getCustomThemeView ({ settingsView }) {
  ...
  const themePackage = atom.packages.getLoadedPackage(`${PACKAGE_NAME}`)
  ...
  let createdPanel = createPanel({ readme, details })
  ...
  panel.element.appendChild(createdPanel)
